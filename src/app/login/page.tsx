          ...
          <form
            action={async () => {
              ...
              if (!process.env.AUTH_SECRET) {
                throw new Error("SETUP ERROR: AUTH_SECRET is missing. Please configure it in your local .env file.");
              }
              if (!process.env.AUTH_GOOGLE_ID || !process.env.AUTH_GOOGLE_SECRET) {
                throw new Error("SETUP ERROR: AUTH_GOOGLE_ID and/or AUTH_GOOGLE_SECRET are missing. Please configure them in your local .env file.");
              }
              await signIn("google");
            }}
          >
            <button type="submit" className="w-full bg-transparent border border-[#262626] text-on-surface rounded-lg py-2 px-4 font-body-md text-body-md hover:bg-surface-container-high transition-colors flex items-center justify-center gap-3">
          ...
          <form
            action={async () => {
              ...
              if (!process.env.AUTH_GITHUB_ID || !process.env.AUTH_GITHUB_SECRET) {
                throw new Error("SETUP ERROR: AUTH_GITHUB_ID and/or AUTH_GITHUB_SECRET are missing. Please configure them in your local .env file.");
              }
              await signIn("github");
            }}
          >
            <button type="submit" className="w-full bg-transparent border border-[#262626] text-on-surface rounded-lg py-2 px-4 font-body-md text-body-md hover:bg-surface-container-high transition-colors flex items-center justify-center gap-3 mt-md">
